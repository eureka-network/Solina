...
fn main() {
    ...
    let message = FF::from_noncanonical_biguint(BigUint::from_bytes_be(message.as_ref()));

    let signature = signature.serialize_compact();
    let s = &signature[..32];
    let r = &signature[32..];
    let r_biguint = BigUint::from_bytes_be(r);
    let s_biguint = BigUint::from_bytes_be(s);
    ...
    let public_key = wallet.get_public_key_coordinate_format();
